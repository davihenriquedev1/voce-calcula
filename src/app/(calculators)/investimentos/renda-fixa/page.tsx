...
import { ComparisonItem, FixedIncomeFormValues } from "@/types/investments/fixed-income";
import ComparisonSummary from "@/components/calculators/investments/fixed-income/ComparisonSummary";
import { Form } from "@/components/ui/form";
import { orchestFixedComparisons } from "@/utils/calculators/investments/fixed-income/orchestFixedComparisons";
...
export default function InvestmentCalculatorPage() {
    ...
    const form = useForm<FixedIncomeFormValues>({ // tipa os values do form que no caso sempre serão strings
        resolver: zodResolver(fixedIncomeSchema), // aplica as validações do schema nos values antes de chamar o handler
        mode: "onTouched",
        criteriaMode: "all",
        defaultValues: {
    ...
    const onSubmit = (values: FixedIncomeFormValues) => {
        setLoading(true);
        setResult(null);

        try {
            const parsed = fixedIncomeSchema.parse(values);
            ...
            const comparisons = orchestFixedComparisons(params);
