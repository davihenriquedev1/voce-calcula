        ...
        <div className={`flex flex-1 flex-col p-4 gap-3 justify-between shadow-lg shadow-black/40 rounded-lg hover:scale-105 transition-all ease-in-out duration-700 ${bgColor ? bgColor : 'bg-gradient-to-r from-slate-50/40 to-slate-200/40'} ${bg ? 'bg-cover bg-center' : ''} lg:flex-row`} style={bg ? { background: `url(${bg})` } : {}}>
            <div className="flex flex-col justify-between h-full">
                <div className="flex-col">
                    <h2 className={`text-md font-semibold mb-2 py-1 px-2 ${colorTitle ? colorTitle : 'text-foreground'} ${bgColorTitle ? bgColorTitle : 'bg-slate-300'}`}>{title}</h2>
                    <p className={`text-sm mb-2 p-1 ${descColor ? descColor : 'text-card-foreground'}`}>{desc}</p>
                </div>
                <div className="flex justify-between">
                    <Button type="button" className="px-0 w-fit bg-transparent py-2 rounded font-bold hover:brightness-105 transition-all">
                        <Link href={route ? route : '/'} className={`flex items-center gap-2 ${linkColor ? linkColor : 'text-foreground'}`} >
                    ...
                    <div className="justify-end items-center flex lg:hidden">
                        <Image width={50} height={50} src={cardIcon as string} alt="" className={` ${cardIconTheme === 'light' || cardIconTheme === 'system' ? 'invert' : ''}`} />
                    </div>
                </div>
            </div>
            <div className="h-full justify-end items-center hidden lg:flex">
                <Image width={130} height={130} src={cardIcon as string} alt="" className={` ${cardIconTheme === 'light' || cardIconTheme === 'system' ? 'invert' : ''}`} />
            </div>
        </div>
    )
}